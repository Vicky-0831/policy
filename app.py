...
LINKS = {
    ...
    "基药目录管理办法(通用版)": BASE_URL + "nhc_jy_glbf.pdf",
    ...
    # 地方级文件
    "北京DRG新药除外支付通知": BASE_URL + "bj_drg.pdf",
    "集采药品接续采购公告(1号)": BASE_URL + "gd_vbp_1.pdf",
    "集采药品接续采购公告(2号)": BASE_URL + "gd_vbp_2.pdf",
    "集采药品接续采购公告(3号)": BASE_URL + "gd_vbp_3.pdf",
    "集采药品接续采购公告(4号)": BASE_URL + "gd_vbp_4.pdf",
    "浙江创新医药支付激励名单": BASE_URL + "zj_incentive.pdf"
}
...
# L2: 内容展示
    ...
    if st.session_state.l1 == "国家":
        dept = st.selectbox("选择部门", ["国家医保局", "国家卫健委"])
        st.markdown(f"### 📂 {dept} 政策集")
        ...
        biz = st.selectbox("选择业务类型", ["国谈落地", "集采", "DRG/DIP", "其他", "超品规备案", "VBP"])
        
        if biz == "国谈落地":
            st.markdown("### 📊 国谈落地指标分析")
            df = load_excel_data()
            if df is None:
                st.warning("⚠️ 根目录下未找到 '数据.xlsx'，请上传后再试。")
            else:
                prov = st.selectbox("请选择省份查询核心指标", df['省份'].unique().tolist())
            ...
            st.markdown("### 📂 集采相关文件 (广东联盟)")
            files = ["集采药品接续采购公告(1号)", "集采药品接续采购公告(2号)", "集采药品接续采购公告(3号)", "集采药品接续采购公告(4号)"]
            st.markdown('<div class="policy-grid">', unsafe_allow_html=True)
            for f in files:
                url = LINKS.get(f, "#")
            ...
            st.markdown("### 📂 DRG/DIP 相关文件 (北京)")
            url = LINKS.get("北京DRG新药除外支付通知")
            st.markdown(f'<div class="file-card"><b>北京第二批DRG付费新药新技术除外支付通知</b><br><a href="{url}" target="_blank" style="font-size:12px; color:#c62828;">🔗 查看原文</a></div>', unsafe_allow_html=True)

        elif biz == "其他":
            st.markdown("### 📂 其他政策文件 (浙江)")
            url = LINKS.get("浙江创新医药支付激励名单")
            st.markdown(f'<div class="file-card"><b>浙江第一批创新医药技术支付激励名单公示</b><br><a href="{url}" target="_blank" style="font-size:12px; color:#c62828;">🔗 查看原文</a></div>', unsafe_allow_html=True)
