...
LINKS = {
    ...
    # 地方性 (带地域标注)
    "【北京】DRG付费新药新技术除外支付工作通知": BASE_URL + "bj_drg.pdf",
...
# L2: 内容呈现
    ...
    if st.session_state.l1 == "国家":
        ...
        st.markdown(f'<div class="sub-header">{dept}</div>', unsafe_allow_html=True)
        ...
        if biz == "国谈落地":
            df = load_excel_data()
            if df is None:
                st.warning("⚠️ 未检测到 '数据.xlsx'，分析模块暂不可用。")
            else:
                prov = st.selectbox("请选择查询省份", df['省份'].unique().tolist())
